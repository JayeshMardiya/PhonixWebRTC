...
class PresenterViewController: UIViewController {
    ...
    func sendOffer(to presenter: String) {
        ...
        self.webRTCClient?.offer { (sdp) in
            ...
            let payload = ["listener_msg": ["sdp": sdp.sdp]]
            // this.channel.push("listener_msg", {a: 1, b: 2, c: 3})
            self.lobbyChannel
                .push("listener_msg", payload: payload)
                .receive("ok") { (message) in
    ...
    private func connectAndJoin() {
        ...
        channel?.onMessage(callback: { message in
            ...
            if let response = message.payload["response"] as? [String: Any] {
                ...
                if let offer = response["offer"] as? [String: Any] {
                    // Accept the offer and send an answer
                    
                    let clientType = offer["clientType"] as? String
                    let sourceId = offer["sourceId"] as? String
                    let destinationId = offer["destinationId"] as? String
                    let strSDP = offer["sdp"] as? RTCSessionDescription
                    
                    let sdp = SDP(with: clientType ?? "", sourceId: sourceId ?? "", destinationId: destinationId ?? "", rtcSDP: strSDP ?? RTCSessionDescription(type: .answer, sdp: offer["sdp"] as? String ?? ""))
                    let rtcSDP = sdp.rtcSDP()
                    let rtcClient = WebRTCClient(iceServers: self.config.webRTCIceServers, and: "presenter", clientId: sdp.sourceId)
                    self.webRTCClient = rtcClient
                    rtcClient.delegate = self
                    rtcClient.set(remoteSdp: rtcSDP) { (error) in
                        self.sendAnswer(to: sdp.sourceId)
                    }
                }
            }
            return message
...
extension PresenterViewController: SignalClientDelegate {
